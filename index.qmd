...
2. How many of the recorded yarns are machine washable?
    ...
    > google: https://www.statology.org/r-count-true-values/ told me that na.rm removes na values. 
...
3. How many missing values are recorded for each variable?
    ...
    > used statology.org a couple of times for finding missing values and how to loop through columns. 
...
missingv<-rep(NA_integer_,times=ncol(yarn))

for(i in 1:ncol(yarn)){
    missingv[i]<-sum(is.na(yarn[,i]))
    print(missingv)
}

# Which column has the most, fewest, and median NA values?
names(yarn)[which.max(missingv)]
names(yarn)[which.min(missingv)]
names(yarn)[missingv==median(missingv)]
...
4. Consider the column `yarn_company_name`.  
    ...
    > Yes, there's a column for each variable and row for each observation and each cell only contains one value. 
...
![q4 sketch](Screenshot 2025-05-08 205009.png)
...
ggplot(common_weight, aes(y=yarn_company_name,x=yarn_weight_name, fill=n))+geom_tile()
...
5. Consider the columns `texture` and `texture_clean`. 
    ...
    > texture has capitalized letters while texture_clean does not.
...
6. Consider the variable `yarn_weight_wpi`.
    ...
    > 1. replace 0-4 with 4 and 5-6 with 6
...
# If you had trouble with 6.4, you can use `prob6pt4.csv` to complete this problem.
yarn|> filter(yarn_company_name=="Bernat") |> summarize(yarn_wt=mean(yarn_weight_wpi_num, na.rm=T))
